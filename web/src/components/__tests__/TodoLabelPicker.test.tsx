...
describe('TodoLabelPicker', () => {
  it('selects several labels without closing and removes one independently', async () => {
    ...
    await user.click(screen.getByRole('button', { name: 'todos.labelsRemove Work' }))
  ...
  it('searches beyond the loaded candidates and remembers the selected name', async () => {
    ...
    await user.keyboard('{Escape}')
    expect(await screen.findByRole('button', { name: `todos.labelsRemove ${remote.name}` })).toBeInTheDocument()
  ...
  it('creates from an empty library and automatically selects the global label', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: 'todos.labelsCreate Remote label' }))
  ...
  it('does not offer a duplicate and supports keyboard multi-selection', async () => {
    ...
    expect(screen.queryByRole('button', { name: /todos.labelsCreate/ })).not.toBeInTheDocument()
  })
  ...
  it('retains the query after creation fails', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: 'todos.labelsCreate New label' }))
    expect(await screen.findByRole('alert')).toHaveTextContent('todos.labelsCreateError')
