...
import { Download, Upload, Network, Bot, CheckCircle, Loader2, Settings2, Cable, Copy, ShieldCheck, ExternalLink, RefreshCw, GripVertical, Clock } from 'lucide-react'
...
// Web session lifetime choices (hours). 0 = never expires (the default).
const SESSION_TTL_CHOICES = [0, 24, 168, 720] as const
...
export default function SettingsPage() {
  ...
  const [sessionTtl, setSessionTtl] = useState<number>(0)
  ...
  // Load captcha + session configs (served by the backend)
  useEffect(() => {
    settingsApi.getCaptcha().then(setCaptchaCfg).catch(() => {})
    settingsApi.getSession().then((cfg) => setSessionTtl(cfg.ttl_hours)).catch(() => {})
  }, [])
  ...
  // Session lifetime: persist, then re-issue the access token right away so
  // the new lifetime applies to the CURRENT session (no re-login needed).
  const handleSaveSession = async () => {
    setSessionSaving(true)
    try {
      const updated = await settingsApi.updateSession({ ttl_hours: sessionTtl })
      setSessionTtl(updated.ttl_hours)
      try {
        await refreshAccessToken()
      } catch {
        // Refresh cookie absent/expired — the setting applies on next login.
      }
      toast.success(t('settings.sessionSaved'))
    } catch {
      toast.error(t('settings.sessionSaveFailed'))
    } finally {
        ...
        <TabsContent value="security" className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <Label>{t('settings.sessionTTL')}</Label>
                <p className="text-sm text-muted-foreground">{t('settings.sessionDesc')}</p>
                <select
                  value={sessionTtl}
                ...
                >
                  {SESSION_TTL_CHOICES.map((h) => (
                    <option key={h} value={h}>
                      {h === 0 ? t('settings.sessionNever')
                        : h === 24 ? t('settings.session1d')
                        : h === 168 ? t('settings.session7d')
                        : t('settings.session30d')}
                    </option>
                  ))}
                </select>
              </div>
              <Button size="sm" onClick={handleSaveSession} disabled={sessionSaving}>
                {sessionSaving && <Loader2 className="size-4 animate-spin" />}
                {t('common.save')}
              </Button>
              <p className="text-xs text-muted-foreground">{t('settings.sessionHint')}</p>
            </CardContent>
          </Card>
        ...
        <TabsContent value="data" className="space-y-6">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              {/* 13 modules × 4 buttons would bury the quick actions above —
                  the per-module matrix lives one click deeper. */}
              <details className="rounded-lg border p-4">
                <summary className="cursor-pointer text-sm font-medium text-primary hover:underline">
                  {t('settings.dataModulesTitle')}
                </summary>
